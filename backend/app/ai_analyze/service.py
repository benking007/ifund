...
DEFAULT_SYSTEM_PROMPT = (
    "你是基金定性分析助手。分析给定数据包后，只输出一个 JSON 对象，"
    "不要任何解释文字，不要用 markdown 代码块包裹。"
    "JSON 的键必须严格匹配下方字段定义，枚举字段只能取规定值。"
)
...
async def _stream_sdk(bundle: dict, system_prompt: str, user_template: str) -> AsyncIterator[str]:
    ...
    opts = QoderAgentOptions(
        ...
        max_turns=1,
        cli_path="/Users/huangcheng/.local/bin/qoder",
    )
